...
// get a single movie by keyword Update 21/07/21
const getByKeyword = function (session, keyword, userId) {
  const query = [
    // 'MATCH (movie:Movie{title:$keyword})',
    'MATCH (movie:Movie)',
  ...
  return session.readTransaction(txc =>
    ...
    .then(result => {
      if (!_.isEmpty(result.records)) {
        ...
        throw {message: 'movie '+keyword+' is not found', status: 404}
      }
    });
};


// get a single movie by plot Update 21/07/21
const getByPlot = function (session, plot, userId) {
  const query = [
    // 'MATCH (movie:Movie{title:$plot})',
    'MATCH (movie:Movie)',
    'WHERE movie.plot CONTAINS $plot',
    'OPTIONAL MATCH (movie)<-[my_rated:RATED]-(me:User {id: $userId})',
    'OPTIONAL MATCH (movie)<-[r:ACTED_IN]-(a:Person)',
    'OPTIONAL MATCH (related:Movie)<--(a:Person) WHERE related <> movie',
    'OPTIONAL MATCH (movie)-[:IN_GENRE]->(genre:Genre)',
  ...
  return session.readTransaction(txc =>
    ...
    .then(result => {
      if (!_.isEmpty(result.records)) {
        ...
        throw {message: 'movie is not found', status: 404}
      }
    });
};
