...
/**
 * @swagger
 * /api/v0/movies/title/{keyword}:
 *   get:
 *     tags:
 *     - movies
 *     description: Find movie by keyword
 *     summary: Find movie by keyword
 *     produces:
 *       - application/json
 *     parameters:
 *       - name: keyword
 *         description: movie keyword
 *         in: path
 *         required: true
 *         type: string
 *       - name: Authorization
...
exports.findByKeyword = function (req, res, next) {
  Movies.getByKeyword(dbUtils.getSession(req), req.params.keyword, req.user.id)
    .then(response => writeResponse(res, response))
    .catch(next);
};
...
/**
 * @swagger
 * /api/v0/movies/plot/{plot}:
 *   get:
 *     tags:
 *     - movies
 *     description: Find movie by plot
 *     summary: Find movie by plot
 *     produces:
 *       - application/json
 *     parameters:
 *       - name: plot
 ...
 *         description: movie not found
 */
 exports.findByPlot = function (req, res, next) {
  Movies.getByPlot(dbUtils.getSession(req), req.params.plot, req.user.id)
    .then(response => writeResponse(res, response))
    .catch(next);
};
/**
